...
### In marketplace portal App Settings:
...
1. User clicks "Connect Platform" in your admin dashboard
2. Frontend calls: `POST /make-server-c5a5d193/oauth/initiate`
3. Server generates state and returns platform authorization URL
4. Frontend redirects user to platform: `https://marketplace.gohighlevel.com/oauth/chooselocation?...`
5. User authorizes and selects location in platform
6. **platform redirects to:** `https://pvophjpndtqxkoygposy.supabase.co/functions/v1/make-server-c5a5d193/oauth/callback?code=xxx&state=xxx`
...
2. ⏳ Update marketplace portal app to use same callback URL
...
Just need to ensure:
1. marketplace portal app uses this exact URL
